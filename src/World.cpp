...
void World::draw_prm_on_obs_mat(){	
		...
		for(int iter=0; iter<this->nodes[i]->get_n_nbrs(); iter++){
			if (this->nodes[i]->get_nbr_i(iter, index)) {
				...
				if (this->nodes[i]->get_nbr_obstacle_cost(iter, obs_cost) && this->nodes[i]->get_nbr_distance(iter, free_dist)) {
					...
					p1.x = (p1.x + this->map_width_meters/2.0);
					p1.y = (p1.y + this->map_height_meters/2.0);
					p2.x = (p2.x + this->map_width_meters/2.0);
					p2.y = (p2.y + this->map_height_meters/2.0);
					cv::line(this->Obs_Mat, p1, p2, pink, 2);
				}
			}
		}
	}
	...
	for (int i = 0; i < this->n_nodes; i++) {
		...
		p1.x = (p1.x + this->map_width_meters/2.0);
		p1.y = (p1.y + this->map_height_meters/2.0);
		cv::circle(this->Obs_Mat, p1, 5, cv::Scalar(255,0,0), -1);
	}
	...
	try{
		if( this->nodes[msg.n_index] ){
		    if( this->nodes[msg.n_index]->is_active() ){
                ...
                if(this->nodes[msg.n_index]->is_active()){
                    if (work_done > 0.0){
		                ROS_INFO("Time(%0.2f):Agent[%i] worked: %0.2f (u) on task[%i]", this->c_time, msg.a_index, work_done, msg.n_index);
                        complete = 1; // Succesfully did some work, but task is still active
                    }
                    else{
		                ROS_WARN("Time(%0.2f):Agent[%i] FAILED to work on task[%i]", this->c_time, msg.a_index, msg.n_index);
                        complete = -1; // Task is active and I did no work
                    }
                }
                else{
                	ROS_INFO("Time(%0.2f):Agent[%i] completed task[%i] and collected %0.2f", this->c_time, msg.a_index, msg.n_index, reward_collected);
...
void World::initialize_nodes_and_tasks() {
	...
	for(int i=int(this->nodes.size()); i < this->n_nodes; i++) {
		...
		int task_type = rand() % n_task_types;
		if(this->hardware_trial){
			if(this->Obs_Mat.at<uchar>(cv::Point(x, y)) <= this->node_obstacle_threshold / 4.0){
